...
import { supabase } from "../Supabase"; // Ensure this path is correct for your setup
...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (error) {
        console.error("Supabase Login Error:", error); // Log the actual Supabase error
        setAlert({ type: "danger", message: error.message });
      } else if (data.user && !data.user.email_confirmed_at) {
        // This check assumes Supabase returns email_confirmed_at in user metadata
        setAlert({ type: "danger", message: "Please verify your email address before logging in." });
      } else {
      ...
      setAlert({ type: "danger", message: "An unexpected error occurred during login." });
    }
  };
